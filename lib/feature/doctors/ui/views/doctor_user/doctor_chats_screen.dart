...
class DoctorChatsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
        ...
        print('Conversations count: ${conversationsByPatientId.length}');

conversationsByPatientId.forEach((k, v) {
  print('Patient ID: $k');
  print('Last Message: ${v.lastMessage}');
  print('Unread: ${v.hasUnreadMessagesFor(doctor.id!)}');
});
    return Scaffold(
      appBar: AppBar(
        title: Text(
            ...
            patients.isEmpty
                ...
                : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _buildPatientCard(patient, conversation, context, doctor);
                  },
                ),
      ),
    );
  ...
  final hasUnread = conversation.hasUnreadMessagesFor(doctor.id!);
...
String _formatTimestampWithElapsed(DateTime time) {
  ...
  final clockTime = '${time.hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}';
